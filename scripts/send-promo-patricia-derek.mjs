...
const targets = [
  { ref: 'CCM-2026-00323', firstName: 'Patricia', lastName: 'Taylor', email: 'patricia955taylor@btinternet.com' },
  { ref: 'CCM-2026-00324', firstName: 'Derek', lastName: 'Handford', email: 'derek.handford@blueyonder.co.uk' },
];

console.log(`Sending to ${targets.length} leads:`);
...
<body style="margin:0;padding:0;background-color:#f5f0e8;font-family:'DM Sans',Arial,sans-serif;">
  ...
  try {
    ...
    if (result.error) {
      ...
      console.log(`✓ Sent to ${firstName} ${lastName} <${email}> — ID: ${result.data?.id}`);
      sent++;
    }
  } catch (e) {
